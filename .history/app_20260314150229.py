...
# ═══════════════════════════════════════════════════════
# MA TYPE — user sets SMA or EMA per stock
# ═══════════════════════════════════════════════════════
@app.route("/set_ma_type", methods=["POST"])
@login_required
def set_ma_type():
    stock_id = request.form.get("stock_id")
    ma_type  = request.form.get("ma_type", "SMA").upper()
    if ma_type not in ("SMA", "EMA"):
        flash("Invalid MA type", "error")
        return redirect(url_for("dashboard"))
    # Verify ownership
    conn  = db.get_db()
    stock = conn.execute("SELECT id FROM watchlist WHERE id=? AND user_id=?",
                         (stock_id, session["user_id"])).fetchone()
    if stock:
        conn.execute("UPDATE watchlist SET ma_type=? WHERE id=?", (ma_type, stock_id))
        conn.commit()
    conn.close()
    flash(f"MA type set to {ma_type}", "success")
    return redirect(url_for("dashboard"))

@app.route("/api/live_ma_typed")
@login_required
def api_live_ma_typed():
    """Extended live_ma that accepts an ma_type param."""
    ticker  = request.args.get("ticker", "").strip()
    ma_type = request.args.get("ma_type", "SMA").upper()
    if not ticker:
        return jsonify({"error": "No ticker"}), 400
    if ma_type not in ("SMA", "EMA"):
        ma_type = "SMA"
    data = engine.get_live_ma_snapshot(ticker, ma_type)
    return jsonify(data)

# ═══════════════════════════════════════════════════════
# CHART DATA — generates OHLCV + indicator JSON
# ═══════════════════════════════════════════════════════
@app.route("/api/chart_data")
@login_required
def api_chart_data():
    """
    Returns full OHLCV + MA + RSI + ADX series for Chart.js rendering.
    Query params: ticker, period (1mo/3mo/6mo/1y), ma_type (SMA/EMA)
    """
    ticker  = request.args.get("ticker", "").strip()
    ...
    if ma_type not in ("SMA", "EMA"):
        ma_type = "SMA"
    data = engine.generate_chart_data(ticker, period, ma_type)
    if not data:
        return jsonify({"error": "Could not fetch chart data"}), 404
    return jsonify(data)

# ═══════════════════════════════════════════════════════
# CHART UPLOAD — user uploads a bar chart image or we save generated one
# ═══════════════════════════════════════════════════════
import base64, uuid

UPLOAD_FOLDER = os.path.join(os.path.dirname(__file__), "static", "charts")
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

@app.route("/api/chart/upload", methods=["POST"])
@login_required
def api_chart_upload():
    """
    Accepts either:
    A) multipart file upload (user uploads their own chart image)
    B) JSON with base64 image data (frontend saves a generated chart screenshot)
    """
    ticker       = request.form.get("ticker") or (request.get_json(silent=True) or {}).get("ticker", "")
    label        = request.form.get("label",  "") or (request.get_json(silent=True) or {}).get("label", "")
    chart_type   = "uploaded"
    filename     = None
    ...
    if "file" in request.files:
        ...
        if f and f.filename:
            ...
            fname    = f"chart_{session['user_id']}_{uuid.uuid4().hex[:8]}{ext}"
            f.save(os.path.join(UPLOAD_FOLDER, fname))
            filename = fname

    # ── B: base64 image from frontend ─────────────────
        ...
        if b64_data:
            ...
            fname    = f"chart_{session['user_id']}_{uuid.uuid4().hex[:8]}.png"
            try:
                with open(os.path.join(UPLOAD_FOLDER, fname), "wb") as out:
    ...
    if not filename:
        return jsonify({"error": "No file or image data received"}), 400

    # Get watchlist_id for this ticker if it exists
    conn  = db.get_db()
    stock = conn.execute("SELECT id FROM watchlist WHERE ticker=? AND user_id=?",
                         (ticker.upper(), session["user_id"])).fetchone()
    wid   = stock["id"] if stock else None
    cur   = conn.cursor()
    cur.execute("""
        INSERT INTO chart_uploads (user_id, watchlist_id, ticker, chart_type, filename, label)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (session["user_id"], wid, ticker.upper(), chart_type, filename, label))
    chart_id = cur.lastrowid
    conn.commit(); conn.close()

    return jsonify({"ok": True, "id": chart_id,
                    "url": f"/static/charts/{filename}", "filename": filename})

@app.route("/api/chart/list")
@login_required
def api_chart_list():
    """Returns all saved charts for the logged-in user."""
    ticker = request.args.get("ticker", "").strip().upper()
    conn   = db.get_db()
    if ticker:
        rows = conn.execute("""
            SELECT * FROM chart_uploads WHERE user_id=? AND ticker=?
            ORDER BY created_at DESC LIMIT 20
        """, (session["user_id"], ticker)).fetchall()
    else:
        rows = conn.execute("""
            SELECT * FROM chart_uploads WHERE user_id=?
...
@login_required
def api_chart_delete(chart_id):
    conn  = db.get_db()
    row   = conn.execute("SELECT * FROM chart_uploads WHERE id=? AND user_id=?",
    ...
    if row:
        ...
        if os.path.exists(fpath):
            os.remove(fpath)
        conn.execute("DELETE FROM chart_uploads WHERE id=?", (chart_id,))
        conn.commit()
    conn.close()
    return jsonify({"ok": True})
